...
const SocialIcon = styled(Icon).attrs(({name}) => ({
  ...
  margin-top: ${({withMarginTop}) =>
    Platform.OS === 'ios' && withMarginTop ? 2 : 0}px;
`;
...
const renderSocialButton = (type: string): Object => {
  const {withMarginTop, iconName, colors, url} = SOCIAL_BUTTONS[type];

  return (
    <ButtonWrapper colors={colors}>
      <SocialButton onPress={() => handleSocialButtonClick(url)}>
        <SocialIcon withMarginTop={withMarginTop} name={iconName} />
      </SocialButton>
    </ButtonWrapper>
  );
};
